...
export const api = {
  ...
  async getMyStore() {
    ...
    try {
      ...
      return { id: snapshot.docs[0].id, ...convertTimestamps(snapshot.docs[0].data()) };
    } catch (error) {
  ...
  async saveStore(data: any) {
    ...
    try {
      const storeData = {
        ...
        createdAt: data.id ? undefined : serverTimestamp(),
      };
      
      if (data.id) {
        await updateDoc(doc(db, 'stores', data.id), storeData);
        return { id: data.id, ...storeData };
      } else {
